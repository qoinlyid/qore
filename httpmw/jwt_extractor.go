...
// extractorLimit is arbitrary number to limit values extractor can return.
// This limits possible resource exhaustion attack vector.
...
func valuesFromQuery(param string) ValuesExtractor {
	return func(c qore.HttpContext) ([]string, error) {
...
// JwtExtractor creates ValuesExtractors from given lookups.
...
func JwtExtractor(lookup string) (extractors []ValuesExtractor, err error) {
	if lookup == "" {
		return
	}

	sources := strings.SplitSeq(lookup, ",")
	for source := range sources {
		parts := strings.Split(source, ":")
		if len(parts) < 2 {
			err = fmt.Errorf(
				"extractor source for lookup %s colud not be split into needed parts: %s",
				lookup, source,
			)
			return
		}
		...
		default: // <- From header should be default.
			prefix := ""
			if len(parts) > 2 {
				prefix = parts[2]
			}
